    ...
    slotchange: e => {
      (e!.target as HTMLSlotElement)
        ...
        .forEach(node => {
          ...
          if ((node as HTMLElement).localName === "datalist") {
            ...
            Array.from(children).forEach(
              (option, idx: number) => {
                const rb = /* html */ `
<input type="radio" name="tabs" id="tab${idx}" value="${(option as HTMLSelectElement).value}">
<div class="tab-label-content" id="tab${idx}-content">
    <label for="tab${idx}">${option.textContent || (option as HTMLSelectElement).value}</label>
</div>`;
                buttons.push(rb);
